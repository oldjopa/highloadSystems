    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uk_aircraft_serial') THEN
            ALTER TABLE aircraft
                ADD CONSTRAINT uk_aircraft_serial UNIQUE (serial_number);
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uk_aircraft_reg') THEN
            ALTER TABLE aircraft
                ADD CONSTRAINT uk_aircraft_reg UNIQUE (registration_number);
        END IF;
    END
$$;
...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uk_deal_number') THEN
            ALTER TABLE deal
                ADD CONSTRAINT uk_deal_number UNIQUE (deal_number);
        END IF;
    END
$$;
...
INSERT INTO deal_status(code, name, description, order_index, is_terminal)
VALUES ('NEW', 'New', 'Newly created', 10, false),
